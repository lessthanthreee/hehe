...
class RSIStrategy:
    def __init__(self, initial_capital=1000, leverage=50):
        ...
        self.min_profit_pct = 2.0  # Minimum profit target (2% with 50x = 100% return)
        self.max_loss_pct = -0.4  # Maximum loss tolerance (-0.4% with 50x = -20% loss)
        self.trailing_stop_pct = 1.0  # Trail the stop loss at 1% below highest price
    ...
    def execute_trade(self, current_price, current_time):
        ...
        if self.position == 0:
            if current_rsi < self.rsi_oversold:  # Oversold condition - Buy signal
                ...
                self.highest_price = current_price  # For trailing stop
        ...
        # If in position, look for sell signals
            ...
            should_sell = (
                ...
                current_price <= trailing_stop_price  # Hit trailing stop
            )
            ...
            if should_sell:
                ...
                sell_reason = "RSI Overbought" if current_rsi > self.rsi_overbought else \
                            ...
                            "Stop Loss Hit" if current_pnl_pct <= self.max_loss_pct else \
                            "Trailing Stop Hit"
