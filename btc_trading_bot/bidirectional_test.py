    ...
    def execute_trade(self, current_price, current_time):
        ...
        if self.position == 0:
            ...
            if long_signal:
                ...
                logging.info(f"LONG Entry - Price: ${current_price:,.2f}, RSI: {rsi:.1f}")
                ...
                logging.info(f"SHORT Entry - Price: ${current_price:,.2f}, RSI: {rsi:.1f}")
                logging.info(f"Position size: ${position_size:,.2f} ({self.leverage}x leverage)")
        ...
        # If in position, look for exit signals
            ...
            if should_exit:
                ...
                logging.info(f"Closing {trade_type} - {exit_reason}")
                logging.info(f"Exit Price: ${current_price:,.2f}, PnL: ${(exit_pnl * self.initial_capital):,.2f} ({current_pnl_pct:.1f}%)")
            ...
            if self.strategy.trades:
                ...
                for i, trade in enumerate(self.strategy.trades, 1):
                    if 'exit_price' in trade:  # Only show completed trades
                        ...
                        logging.info(f"\nTrade #{i} ({trade['type']}):")
                        logging.info(f"Entry: ${trade['entry_price']:,.2f} (RSI: {trade['entry_rsi']:.1f})")
                        logging.info(f"Exit: ${trade['exit_price']:,.2f}")
                        logging.info(f"Reason: {trade['exit_reason']}")
                        logging.info(f"PnL: ${pnl:.2f} ({trade['pnl_pct']:.1f}%)")
