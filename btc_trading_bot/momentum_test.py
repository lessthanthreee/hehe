...
from momentum_divergence_strategy import MomentumDivergenceStrategy

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

class MomentumTester:
    def __init__(self):
        self.strategy = MomentumDivergenceStrategy(
            initial_capital=200,
            leverage=50,
            rsi_period=14,
            ema_fast=8,
            ema_slow=21,
            profit_target=0.004,  # 0.4% profit
            stop_loss=-0.0015  # 0.15% stop
        )
        self.running = True
        self.ws = None
    ...
    def run(self, duration_hours=10):
        """Run strategy for specified duration"""
        try:
            logging.info("\nStarting Momentum Divergence Test")
